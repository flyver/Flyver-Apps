...
public class FlyverMQ {
    ...
    private Semaphore topicsGuard = new Semaphore(1);

    public FlyverMQ() {
    }

    public static FlyverMQ getInstance() throws FlyverMQException {
        if (instance == null) {
            throw new FlyverMQException("FlyverMQ is not instantiated");
        }
        return instance;
    }
...
//            producers.get(topic).unregistered();
    ...
    protected boolean addMessage(FlyverMQProducer producer, FlyverMQMessage data) throws NoSuchTopicException {
        if (!topics.containsKey(producer.topic)) {
            throw new NoSuchTopicException(NOTOPICMSG + producer.topic);
        }
